...
import getFirstImageFromRichTextDataOrNull from "../utils/getFirstImageFromRichTextDataOrNull"

export const query = graphql`
query  {
  site: allSanityStaticSite(filter:{staticSiteSlug: {current: {eq: "ueber"}}}) {
    nodes {
  ...
  getFirstPictureStaticSite: sanityStaticSite(staticSiteSlug: {current: {eq: "ueber"}}) {
    _rawStaticSiteContent(resolveReferences: {maxDepth: 10})
  }
}
`

const ueber = ( {data, location} ) => {
  ...
  return (
    ...
    <div className="card shadow kg-card mt-5 mb-4">
      <CardHeaderBanner title={site.title} subtitle={site.intro} type='bio' />
    </div>
    {site && (
      <div className="card shadow mb-4 about-styling" style={{padding:'1rem'}}>
